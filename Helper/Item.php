...
/**
 * Class Item
 * @package Aceextensions\OneStepCheckout\Helper
 */
class Item extends Data
{
    ...
    public function getItemOptionsConfig($quote, $item)
    {
        ...
        if ($product->getOptions()) {
            /** @var Options $options */
            $options = $this->getLayout()->getBlock('onestepcheckout.product.options');
    ...
    private function getConfigurableConfig($item, $product)
    {
        ...
        $block = $this->getLayout()->getBlock('onestepcheckout.configurable.options');
    ...
    {
        /** @var Bundle $block */
        $block = $this->getLayout()->getBlock('onestepcheckout.bundle.options');
        $block->setData([
            'product' => $item->getProduct(),
            'item' => $item
        ]);
    ...
    {
        /** @var Renderer $block */
        $block = $this->getLayout()->getBlock('onestepcheckout.downloadable.options');
        $block->setData([
            'product' => $item->getProduct(),
            'item' => $item
        ]);
    ...
    {
        /** @var Giftcard $block */
        if (!$block = $this->getLayout()->getBlock('onestepcheckout.giftcard.options')) {
            $block = $this->getLayout()->createBlock(
                Giftcard::class,
                'onestepcheckout.giftcard.options'
            );
        }

        $block->setTemplate('Aceextensions_OneStepCheckout::product/view/type/options/giftcard.phtml');
        $block->setData([
            'product' => $item->getProduct(),
            'item' => $item
        ]);
