...
use Aceextensions\OneStepCheckout\Api\Data\OneStepCheckoutDetailsInterface;
use Aceextensions\OneStepCheckout\Helper\Item as OneStepCheckoutHelper;
...
class CheckoutManagement implements CheckoutManagementInterface
{
    ...
    /**
     * @var OneStepCheckoutDetailsFactory
     */
    protected $OneStepCheckoutDetailsFactory;
    ...
    /**
     * @var OneStepCheckoutHelper
     */
    protected $OneStepCheckoutHelper;
     ...
     * @param OneStepCheckoutDetailsFactory $OneStepCheckoutDetailsFactory
     ...
     * @param OneStepCheckoutHelper $OneStepCheckoutHelper
    ...
    public function __construct(
        ...
        DetailsFactory $OneStepCheckoutDetailsFactory,
        ...
        OneStepCheckoutHelper $OneStepCheckoutHelper,
        Message $giftMessage,
        ...
        $this->OneStepCheckoutDetailsFactory = $OneStepCheckoutDetailsFactory;
        ...
        $this->OneStepCheckoutHelper = $OneStepCheckoutHelper;
     ...
     * @param Quote $quote
     *
     * @return OneStepCheckoutDetailsInterface
     * @throws NoSuchEntityException
     */
    public function getResponseData(Quote $quote)
    {
        /** @var OneStepCheckoutDetailsInterface $OneStepCheckoutDetails */
        $OneStepCheckoutDetails = $this->OneStepCheckoutDetailsFactory->create();

        if (!$quote->hasItems() || $quote->getHasError() || !$quote->validateMinimumAmount()) {
            return $OneStepCheckoutDetails->setRedirectUrl($this->_urlBuilder->getUrl('checkout/cart'));
        }

        if ($quote->getShippingAddress()->getCountryId()) {
            $OneStepCheckoutDetails->setShippingMethods($this->getShippingMethods($quote));
        }

        $OneStepCheckoutDetails->setPaymentMethods($this->paymentMethodManagement->getList($quote->getId()));
        $OneStepCheckoutDetails->setTotals($this->cartTotalsRepository->get($quote->getId()));
        ...
        foreach ($quote->getAllVisibleItems() as $item) {
            ...
            $optionsData[$item->getId()] = $this->OneStepCheckoutHelper->getItemOptionsConfig($quote, $item);
            $imageData[$item->getId()] = $this->OneStepCheckoutHelper->getItemImages($item);
            $requestPath[$item->getId()] = $product->getUrlModel()->getUrl($product);
        }

        $OneStepCheckoutDetails
            ->setImageData($this->OneStepCheckoutHelper->jsonEncodeData($imageData))
            ->setOptions($this->OneStepCheckoutHelper->jsonEncodeData($optionsData))
            ->setRequestPath($this->OneStepCheckoutHelper->jsonEncodeData($requestPath));

        return $OneStepCheckoutDetails;
    }
    ...
    public function saveCheckoutInformation(
        ...
        try {
            ...
            $this->checkoutSession->setOneStepCheckoutData($additionInformation);
    ...
    public function addGiftMessage($cartId, $additionInformation)
    {
        ...
        if (isset($additionInformation['giftMessage']) && !$this->OneStepCheckoutHelper->isDisabledGiftMessage()) {
            $giftMessage = $this->OneStepCheckoutHelper->jsonDecodeData($additionInformation['giftMessage']);
